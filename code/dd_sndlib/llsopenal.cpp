      ...
      /*else if (SoundEntries[i].info && (state == AL_PLAYING || SoundEntries[i].streaming))
      {
              ...
              __except (GetExceptionCode() == EXCEPTION_ACCESS_VIOLATION)
              {
                      ...
                      if (SoundEntries[i].streaming)
                              ...
                              PutLog(LogLevel::Error, "\tSound num is %d (%s)", SoundEntries[i].soundNum,
      pSounds[SoundEntries[i].soundNum].name);

                      //MessageBox(app->m_hWnd, TEXT("Access violation in sound system, please show ISB
      InjectD3Output.txt"), TEXT("Sound system error"), MB_OK);
              }
      }*/
    }
  }
